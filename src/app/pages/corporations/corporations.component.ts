...
@Component({
  ...
  styles: [require('./corporations.scss'),require('../css/ng2-toastr.min.scss')],
  template: require('./corporations.html'),
...
export class CorporationsComponent implements OnInit {
  ...
  constructor(private _corporationService: CorporationsService, private dialogService:DialogService, public toastr: ToastsManager, vcr: ViewContainerRef, 
  ...
  addDealflow(id:Number,listName:String) {
    ...
    getPage(page: number) {
        ...
        if(this.filters.length > 0){
            ...
            /* this._corporationService.getVenturesPageFilter(page, this.filters).map(res => {
                // If request fails, throw an Error that will be caught
                if(res.status == 204) {
                    this.loading = false;
                    this.error = true;
            ...
            /* this._corporationService.getVenturesPage(page, this.searchString).map(res => {
                // If request fails, throw an Error that will be caught
                if(res.status == 204) {
                    this.loading = false;
                    this.error = true;
